...
def carregarmodelo(path):
    """
    Carrega o modelo treinado
    :param path: caminho do arquivo que contém o modelo
    :param filename: nome do arquivo
    :return: modelo_treinado
    """
    modelo_treinado = joblib.load(path)
    
    return modelo_treinado 

def fazerpredicoes(modelo, dataset):
    """
    Faz previsões a partir dos dados
    :param modelo: caminho do arquivo que contém os dados de venda
    :param dataset: caminho do arquivo que contém os dados de temperatura
    :return: datasets contendo os dados e a previsão
    """
    # Dropando ausentes
    dataset = dataset.dropna()
    ...
    predicoes = modelo.predict(dataset)
    dataset['predicao'] = predicoes
    
    return dataset
    
def salvardados(dataset, path):
    """
...
def main():
    if len(sys.argv) == 6:
        ...
        vendas_path, temperatura_path, estoque_path, modelo_path, resultados_path = sys.argv[1:]
        ...
        modelo = carregarmodelo(modelo_path)
        
        # Realizando predições
        print('Realizando predições.................')
        dataset = fazerpredicoes(modelo, dataset_limpo)

        # Salvando os dados
        print('Salvando os dados................')
        salvardados(dataset, resultados_path)
